import os
import json
from services.triz.triz_constants import (
    TRIZ_PRINCIPLES,
...
def create_triz_data_files():
    ...
    if isinstance(ENGINEERING_PARAMETERS, dict):
        ...
        print(
            f"WARNING: Unexpected ENGINEERING_PARAMETERS type: {type(ENGINEERING_PARAMETERS)}"
        )
            ...
            if isinstance(worsening_dict, dict):
                for worsening_param, principles in worsening_dict.items():
                    ...
                    if isinstance(worsening_param, str) and worsening_param.isdigit():
                        ...
                        isinstance(worsening_param, str)
                        and worsening_param in param_names
                    ):
                        col_idx = param_names[worsening_param]
                    else:
                        print(
                            f"WARNING: Could not map worsening parameter: {worsening_param}"
                        )
                        continue
