...
def get_all_patents(
    search_term: str = "", status_filter: List[str] = None, sort_order: str = "newest"
) -> List[Dict[str, Any]]:
    """
    ...
    if search_term:
        ...
        patents = [
            ...
            if search_term in patent["title"].lower()
            or search_term in patent.get("abstract", "").lower()
            or search_term in patent.get("inventor", "").lower()
            or search_term in patent.get("assignee", "").lower()
        ]

    # Apply status filter if provided
    if status_filter:
        patents = [
            patent for patent in patents if patent.get("status") in status_filter
        ]
...
def get_patent_citations(patent_id: str) -> List[Dict[str, Any]]:
    ...
    citations = [
        ...
        if citation["patent_id"] == patent_id
    ]
...
def get_analyses_for_patent(patent_id: str) -> List[Dict[str, Any]]:
    ...
    analyses = [
        ...
        if analysis["patent_id"] == patent_id
    ]
...
def analyze_patent(patent_filename: str) -> Dict[str, Any]:
    ...
    try:
        ...
        new_analysis = {
            ...
            "extracted_data": {"triz_contradictions": triz_analysis},
            "feedback_date": None,
            "user_feedback": None,
            "status": "new",
        }
...
def upload_patent(
    file_data: bytes, filename: str, metadata: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Upload a new patent.
    ...
    new_patent = {
        ...
        "filing_date": metadata.get(
            "filing_date", serialize_datetime(datetime.datetime.now())
        ),
...
def create_patent_with_url(file_url, file_name, metadata=None):
    """
    Create a new patent entry with a file URL instead of uploading a file.
    
    Args:
        file_url (str): URL to the patent PDF file in CDN/storage
        file_name (str): Original filename of the PDF
        metadata (dict): Additional patent metadata like title, inventors, etc.
        
    Returns:
        dict: The created patent data
    """
    try:
        from patent_api.models import Patent
        from datetime import datetime
        
        # Extract metadata or use defaults
        metadata = metadata or {}
        
        # Create the patent record with the file URL
        patent = Patent(
            patent_number=metadata.get('patent_number', f"US{datetime.now().strftime('%Y%m%d%H%M%S')}"),
            title=metadata.get('title', file_name),
            abstract=metadata.get('abstract', 'No abstract provided'),
            filing_date=metadata.get('filing_date', datetime.now().date()),
            publication_date=metadata.get('publication_date', datetime.now().date()),
            inventors=metadata.get('inventors', 'Unknown'),
        ...
        return {
            ...
            'pdf_file': patent.pdf_file,
            'pdf_file_name': patent.pdf_file_name,
            'upload_date': patent.upload_date.isoformat() if patent.upload_date else None,
        }
        
    except Exception as e:
        import traceback
        traceback.print_exc()
        raise Exception(f"Failed to create patent with URL: {str(e)}")
