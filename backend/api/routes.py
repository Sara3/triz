...
def register_routes(app):
    ...
    def health_check():
        ...
        return jsonify({
            'status': 'ok',
            'message': 'Patent Analytics Hub API is running'
        })
    ...
    def upload_patent_route():
        ...
        try:
            ...
            if file:
                ...
                metadata = {}
                for field in ['title', 'abstract', 'assignee', 'inventors', 'classification']:
                ...
                if 'inventors' in metadata and isinstance(metadata['inventors'], str):
                    metadata['inventors'] = [inv.strip() for inv in metadata['inventors'].split(',')]
                
                # Generate a unique ID
                patent_id = f"PAT{uuid.uuid4().hex[:6].upper()}"
                metadata['id'] = patent_id
    ...
    def delete_analysis_route(analysis_id):
        ...
        try:
            ...
            if success:
                return jsonify({'message': f'Analysis {analysis_id} deleted successfully'})
